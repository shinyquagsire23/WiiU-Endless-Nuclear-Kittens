...
float global_rot_ox = 0.0f;
float global_rot_oy = 0.0f;

void clean_tex_allocs()
{
    if(!tex_allocs)
    {
        tex_allocs = calloc(0x20000, sizeof(float*));
        return;
    }
...
float get_x_rot(float px, float py)
{
    return cos(global_rotation) * (px-global_rot_ox) - sin(global_rotation) * (py-global_rot_oy) + global_rot_ox;
}

float get_y_rot(float px, float py)
{
    return sin(global_rotation) * (px-global_rot_ox) + cos(global_rotation) * (py-global_rot_oy) + global_rot_oy;
}
...
void render_texture_partial_color(GX2Texture *render_texture, float x_pos, float y_pos, float width, float height, int partial_x, int partial_y, int partial_width, int partial_height, float r, float g, float b, float a)
{
    ...
    //TODO: Maybe not do this? idk
    global_rot_ox = (x_pos+width/2);
    global_rot_oy = (y_pos+height/2);
    ...
    {
         ((get_x_rot(x_pos, y_pos) / (float)TARGET_WIDTH) * 2.0f)-1.0f, ((get_y_rot(x_pos, y_pos) / (float)TARGET_HEIGHT) * 2.0f)-1.0f, 0.0f,
         ((get_x_rot(x_pos, y_pos+height) / (float)TARGET_WIDTH) * 2.0f)-1.0f, ((get_y_rot(x_pos, y_pos+height) / (float)TARGET_HEIGHT) * 2.0f)-1.0f, 0.0f,
         ((get_x_rot(x_pos+width, y_pos) / (float)TARGET_WIDTH) * 2.0f)-1.0f, ((get_y_rot(x_pos+width, y_pos) / (float)TARGET_HEIGHT) * 2.0f)-1.0f, 0.0f,
         
         ((get_x_rot(x_pos+width, y_pos+height) / (float)TARGET_WIDTH) * 2.0f)-1.0f, ((get_y_rot(x_pos+width, y_pos+height) / (float)TARGET_HEIGHT) * 2.0f)-1.0f, 0.0f,
    };
